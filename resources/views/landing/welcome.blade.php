<x-guest-layout>
        ...
        <section class=" w-full h-screen bg-fixed  image1  image  ">
            <div class="  flex flex-col justify-center items-center w-full lg:w-2/3 ml-auto h-4/5 HomeContent">
                <div class="mt-8 w-full lg:w-2/3  bg-gray-400  p-16 rounded-2xl bg-opacity-40">
                    <h1 class="text-4xl lg:text-4xl text-white text-center lg:text-left  lg:mb-4">Welcome to Explore Kenya </h1>
                    <h3 class="text-2xl lg:text-2xl text-white text-right mt-2">Your journey starts here</h3>
                    <p class="  text-white text-xl text-center content-para mt-8 ">Discover Counties ,Cities, Towns and
                        magnificent locations in Kenya. Use the blog to interact with our community and many great features</p>
                </div>
                <div class="">
                    @if (!Auth::user())
                    ...
                    <div class="h-2/3  md:h-full md:w-2/3 p-6 md:p-12 md:pb-0">
                        <div class="">
                            ...
                            <p class="font-semibold text-base md:text-lg mt-4">use our app to get the exact location for
                                hotels , restaurants , cities, towns and points of interest</p>
                        </div>
                    </div>
                    {{-- svg div --}}
                    <div class=" self-end">
                        ...
                        <div class=" mb-8">
                            @if (!auth()->user())
                                ...
                                <p class="font-semibold text-xl">or <a href="{{ route('register') }}"
                                    ...
                                    now !</p>
                            @else
                                <p class="font-semibold text-xl">or <a href="/blog"
    ...
    <x-slot name="countiesSection">
        {{-- supported counties section --}}
       @livewire('object-container' , ['collection'=>$counties , 'type'=> 'counties'])
    </x-slot>
